...
def main(args):
    ...
    if args.use_random_buffer is True:
        buffer4model = Buffer(state_dim=state_dim, action_dim=action_dim, max_size=args.buffer_size,
                                   on_policy=False, device=algorithm.device)
    else:
        buffer4model = None
        ...
        if i%args.eval_step == 0 and i != 0:
            ...
            while True:
                ...
                while not done:
                    ...
                    error_list = eval_models(observation, action, next_observation, models)
