...
model_list = ["model" + name + "_" + args_itg.model_kind for name in args_itg.model_order]
...
if args_itg.develop_mode != 'DeepDOB':
    ...
    for model in model_list:
        ...
        for i in np.linspace(start_mag, mag_range, args_itg.num_dist+1):
            ...
            saveData = np.array([reward_avg, reward_max, reward_min, reward_std, alive_rate])
            Tester_data.put_data(saveData)
        Tester_data.save_data(args_itg.save_dir, time.strftime("%m%d_") + args_itg.result_fname + '_' + model + '_' + args_itg.dist_kind+'2'+args_itg.add_to)
